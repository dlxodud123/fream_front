...
const Sell = () => {
  ...
  const transformData = (data) => {
    ...
    return data.map((item) => {
      console.log("orderDate:", item);
      const formattedOrderDate = item.orderDate
        ? format(parseISO(item.orderDate), "yyyy/MM/dd")
        : "N/A";
  ...
  const sellListData = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setSellingData(transformedData);
      });
  };

  useEffect(() => {
    sellListData(); // 컴포넌트가 마운트될 때 데이터가지고옴
  }, []);
  useEffect(() => {
    openModalTender();
  }, []);
    ...
    <div>
      ...
      <div className="container_box">
        ...
        <div className="box-container">
          ...
          <div>
            {isModalTenderOpen && (
              <Selling onClose={closeModalTender} buyBreakdown={sellingData} />
            )}
            {isModalProgresOpen && (
              <KanbanSell
                onClose={closeModalProgres}
                buyBreakdown={sellingData}
              />
            )}
            {isModalDealEndOpen && (
              <SellEnd onClose={closeModalDealEnd} buyBreakdown={sellingData} />
            )}
          </div>
        </div>
      </div>
